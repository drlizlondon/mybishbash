...
drop constraint if exists bishbash_state_pkey;
...
alter table public.bishbash_state
add constraint bishbash_state_pkey primary key (id);

-- Preserve old sync-code uniqueness
alter table public.bishbash_state
drop constraint if exists bishbash_state_profile_id_key;

alter table public.bishbash_state
add constraint bishbash_state_profile_id_key unique (profile_id);